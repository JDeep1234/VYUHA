...
public:
   ...
   */
  ExecutionResult
  executeTechnique(const std::string &techniqueId,
  ...
  /**
   * Set execution state (called internally and by agent coordination)
   */
...
public:
   ...
   */
  bool exportSTIX(const std::string &filepath,
                  const std::vector<ExecutionResult> &results);
...
public:
   ...
   */
  ExecutionResult
  runTechnique(const std::string &techniqueId,
               const std::map<std::string, std::string> &options);
  ...
  runCampaign(const std::string &campaignFile,
