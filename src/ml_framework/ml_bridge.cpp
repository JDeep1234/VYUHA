...
std::vector<float> SystemState::toVector() const {
    ...
    for (float f : lastAction)       v.push_back(f);
    // Previous result (4 one-hot)
    for (float f : lastActionResult) v.push_back(f);
    // Counters (2)
    v.push_back(consecutiveFailures);
    v.push_back(timeSinceLastAction);

    // Ensure exactly 30 elements
    while (v.size() < 30) v.push_back(0.f);
    if (v.size() > 30)    v.resize(30);
    return v;
}

SystemState SystemState::fromContext(
    const std::string& edrName, const std::string& /*edrVer*/,
    bool edrRunning, bool driverLoaded, int winBuild, int intLevel,
    bool ppl, bool dse, bool sb, bool virt, bool elam,
    ...
    int consecFails, double secsSinceLast)
{
    ...
    // EDR product one-hot
    static const char* EDR_NAMES[] = {"defender","crowdstrike","carbonblack","sophos","other"};
    ...
    if (winBuild >= 22000)      s.windowsVersion[1] = 1.f;  // Win11
    else if (winBuild >= 19041) s.windowsVersion[0] = 1.f;  // Win10
    else if (winBuild >= 17763) s.windowsVersion[2] = 1.f;  // Server 2019
    else                        s.windowsVersion[3] = 1.f;  // Server 2022+/other
    ...
    if (intLevel >= 0 && intLevel <= 2) s.integrityLevel[intLevel] = 1.f;
    ...
    s.timeSinceLastAction  = std::min(1.f, static_cast<float>(secsSinceLast) / 300.f);
...
std::string MLBridge::jsonGetString(const std::string& json, const std::string& key) {
...
bool MLBridge::start(const std::string& pythonPath, const std::string& scriptPath) {
    ...
    if (script.empty()) {
        ...
        if (script.empty()) {
            std::cerr << "[MLBridge] Could not find ml_server.py. "
                         "Set scriptPath explicitly." << std::endl;
            return false;
        }
    }
    ...
    int fdOut = _open_osfhandle(reinterpret_cast<intptr_t>(hChildStdout_Rd), _O_RDONLY);
...
void MLBridge::stop() {
    ...
    if (toChild_)   { fclose(toChild_);   toChild_   = nullptr; }
    if (fromChild_) { fclose(fromChild_); fromChild_ = nullptr; }
...
TrainResponse MLBridge::train(const SystemState& state, int action, float reward,
                               const SystemState& nextState, bool done) {
    auto sv  = state.toVector();
    auto nsv = nextState.toVector();

    std::ostringstream oss;
    oss << "{\"command\":\"train\""
        << ",\"state\":"      << vectorToJsonArray(sv)
        << ",\"action\":"     << action
        << ",\"reward\":"     << reward
...
ClusterResponse MLBridge::clusterEDR(const std::map<std::string, float>& features,
    ...
    if (start != std::string::npos) {
        ...
        if (end != std::string::npos) {
            ...
            while ((p = arr.find('[', p)) != std::string::npos) {
                size_t q = arr.find(',', p);
                size_t r = arr.find(']', q);
                if (q == std::string::npos || r == std::string::npos) break;
                std::string nameRaw  = arr.substr(p + 1, q - p - 1);
                std::string scoreRaw = arr.substr(q + 1, r - q - 1);
                // Strip quotes
                nameRaw.erase(std::remove(nameRaw.begin(), nameRaw.end(), '"'), nameRaw.end());
                try {
...
Outcome classifyOutcome(bool success, bool detected, const std::string& output) {
    ...
    if (crashed)  return Outcome::SYSTEM_CRASH;
    if (!success && detected) return Outcome::ACTION_BLOCKED;
    if (!success) return Outcome::NO_CHANGE;
    if (success && detected)  return Outcome::PARTIAL_SUCCESS;   // ran but EDR saw it

    bool telemetry = lower.find("telemetry") != std::string::npos ||
                     lower.find("etw")       != std::string::npos;
    if (success && telemetry) return Outcome::TELEMETRY_SILENCED;
    if (success)              return Outcome::EDR_TERMINATED;

    return Outcome::NO_CHANGE;
}

float calculate(bool success, bool detected,
                const std::string& output, float stealthScore) {
    static const std::map<Outcome, float> BASE_REWARD = {
        {Outcome::EDR_TERMINATED,    100.f},
        {Outcome::TELEMETRY_SILENCED, 75.f},
        {Outcome::PARTIAL_SUCCESS,    50.f},
        {Outcome::NO_CHANGE,           0.f},
        {Outcome::DETECTED_ALERT,    -50.f},
        {Outcome::ACTION_BLOCKED,    -75.f},
